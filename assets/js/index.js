...
const verificarLetters = (letter, string) => {
...
function startGame() {
    ...
    theme.innerHTML = palavras[index].tema
}

function resetGame() {
    // Função que reinicia o navegador.
...
function randomIndex() {
    ...
    index_palavra = Math.floor(Math.random() * palavras[index].palavra.length)
}
...
function addSublinhado() {
    ...
    for (let i=div.childElementCount; i < palavras[index].palavra[index_palavra].length; i++) {
...
function addSublinhadoArray() {
    ...
    for (let i=letters.length; i < palavras[index].palavra[index_palavra].length; i++) {
        letters.push("_")
    }
}
...
function LetterButtonClick(event) {
    ...
    const stringPalavra = palavras[index].palavra[index_palavra].toLowerCase()
